...
type minted1 = {
  ...
  metadata_big_map: nft_meta;
  reversed_txs : transfer_destination_descriptor list;
}

let update_meta_and_create_txs (param, storage, metadata_big_map
    : mint_tokens_param * nft_token_storage * nft_meta) : minted1 =
  let seed1 : minted1 = {
    storage = storage;
    metadata_big_map = metadata_big_map;
    reversed_txs = ([] : transfer_destination_descriptor list);
  } in
  List.fold
    (fun (acc, t : minted1 * mint_token_param) ->
      ...
      then (failwith "FA2_INVALID_TOKEN_ID" : minted1)
        ...
        let new_token_metadata =
          Big_map.add new_token_id t.token_metadata acc.metadata_big_map in
        ...
        {
          storage = new_storage;
          metadata_big_map = new_token_metadata;
          reversed_txs = tx :: acc.reversed_txs;
        }
    ) param seed1

let mint_tokens (param, storage, metadata_big_map : mint_tokens_param * nft_token_storage * nft_meta)
    : operation list * nft_token_storage * nft_meta =
  let mint1 = update_meta_and_create_txs (param, storage, metadata_big_map) in
  (* update ledger *)
  ...
  ops, storage, mint1.metadata_big_map
